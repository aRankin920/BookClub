...
<body style="background-image: url('/images/bk3.jpg');background-repeat: no-repeat; background-size: cover">
		 ...
		 <div class="gridContainerDashboard">	
		   <div class="bookRunner">			   
			  <div class="listFormat">
			   ...
			   <ul>
			       <c:forEach items="${bkProdList}" var="product" begin="1" end="8">
                   <li>  <c:out value="${product.getTitle()}"/> </li>
                   </c:forEach>
			   </ul>
	   ...
	   <div class = "topNavDashboard">
		      ...
		      <div  class = "innertopNavDashboard">	
			      <form:form action="/bookclub/reviews" method="post" modelAttribute="reviews">	
				 	...
				 	<form:textarea path = "reviewInput" rows = "7" cols = "110" style="margin-left:25px; " placeholder="${ defaultValue }" />
					<br><br>
           ...
           <div style="margin-left:25px; text-align:left;">
		      	 <span style="font-weight:bold; font-size: 23px;">See member's comments:</span>      	  
		         <p>
				    <c:forEach items="${reviewList}" var="reviews">
		          <form:form action="/bookclub/update/${reviews.id}" method="post" modelAttribute="reviews">	
				         
				         <form:input  path="reviewInput"  type="text" value = "${reviews.getReviewInput()}" />				    											   
						 <form:errors path="reviewInput"/>
						
<!-- 						  <button type="submit" value="Update"/> -->
						  	<button type="submit" >Update</button>
						 					 		
						 <a href="/bookclub/delete/${reviews.getId()}">Delete</a><br><br>
					</form:form>
					</c:forEach>
		         </p>
		  </div>
